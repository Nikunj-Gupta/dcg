...
class GNNFeatureAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(GNNFeatureAgent, self).__init__()
        self.args = args
        ...
        if agent == "gcn_feat": self.conv1 = GCNConv(in_channels, out_channels) 
        if agent == "gat_feat": self.conv1 = GATConv(in_channels, out_channels) 
        if agent == "gatv2_feat": self.conv1 = GATv2Conv(in_channels, out_channels) 
